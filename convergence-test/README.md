...
```
$ cargo run --bin convergence-test --release -- path -o ratios -v step-dir | tee ratios.csv
```
...
```
$ cargo run --bin convergence-test --release -- path -v step-dir | tee step-err.csv
$ cargo run --bin convergence-test --release -- path -v deriv-dir | tee deriv-err.csv
$ cargo run --bin convergence-test --release -- path -v point | tee point-err.csv
```
...
Before doing that, though, we want a rough idea of the step-level
error we're aiming for, and for that, the usual path length.

I extended the `convergence-test` tool to report path lengths:

```
$ cargo run --bin convergence-test --release -- path -v path-len | sed 's/,/\n/g' | sort -n | tail -n 1
5.27999999754568
```

This longest length was bounded below by the infinity radius of 4.0,
so this seems roughly the right number. With a conservative path
length of 10.0, and an error in the final direction of 1/10000, we
want a per unit travelled error of 1e-5 or less.

(This is, of course, assuming the worst case that the step errors
accumulate, ratehr than cancel each other out.)

I then added a per-step error information mode to the
`convergence-test` tool, and used it to find a good proxy for the
actual error seen. This was actually pretty messy trial-and-error, so
I'll present a sanitised version below.

To create a baseline "correct" result for each step to calculate the
error from, each step is shadowed by a path made of 10 steps of 1/10th
the size, and the error is taken to be the distance between these
points, as a fraction of the step length.

I then came up with three error proxies:

 * **Curvature** How far the new point was away from the straight-line
   extrapolation, as deflection per unit length.
 * **Derivative of curvature** Taken as distance between the normals
   at either end of the step, per unit length.
 * **Movement from normal change** We usually project into the surface
   with the normal from the start of the step. What if we project the
   same distance, but using the normal at the end of the step? This
   proxy uses the distance between the two points, reflecting
   (roughly) the distance moved if we switch the normal used.

Analysing the data, "movement from normal change" was the best proxy.

I want to analyse how the error varies with the proxy, but millions of
data points are hard to plot, so I sort by proxy/error ratio to get a
reasonable distribution, and then evenly chop down.

```
$ cargo run --release --bin convergence-test -- step norm-mvmt -s 0.01 > step-stats-0.01.csv
$ cargo run --release --bin convergence-test -- step norm-mvmt -s 0.02 > step-stats-0.02.csv
$ cargo run --release --bin convergence-test -- step norm-mvmt -s 0.04 > step-stats-0.04.csv
$ cargo run --release --bin convergence-test -- step -norm-mvmt s 0.08 > step-stats-0.08.csv
$ wc -l step-stats-0.01.csv
 1920773 step-stats-0.01.csv
...
When the proxy returns a very small value, there seems to be another
source of error that contributes (the second line on the left of the
